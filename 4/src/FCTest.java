...
public class FCTest {
    ...
    private static void findPlayer(final FootballClub fc) {
        final String playerFirstNave = scanner.next();
        final int jerseyNumber = scanner.nextInt();
        final Player foundPlayer = fc.findPlayer(playerFirstNave, jerseyNumber);
        if(foundPlayer != null) {
            System.out.println(foundPlayer);
        }
        else {
            System.out.println("Player Not Found with name " + playerFirstNave + " and number " + jerseyNumber);
        }
    }
